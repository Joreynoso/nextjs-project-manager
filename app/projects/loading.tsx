  ...
  return (
    <div className='w-full min-h-[calc(100vh-4rem)] mx-auto flex flex-col py-5 pb-10 max-w-7xl'>
       ...
       <div className="mb-5 flex flex-col gap-y-2">
         ...
         <Skeleton className="h-5 w-64 mt-2 bg-secondary" /> {/* Descripción fake */}
           </div>

       <div className="flex justify-between items-center mb-6">
          <Skeleton className="h-10 w-40 bg-secondary" /> {/* Botón agregar fake */}
       </div>

       {/* Grid de skeletons simulando tarjetas */}
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1, 2, 3, 4, 5, 6].map((i) => (
             <Skeleton key={i} className="h-[300px] w-full rounded-xl bg-secondary" />
          ))}
       </div>
    </div>
  )
